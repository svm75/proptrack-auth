...
import {
  ...
  ownerOccupancyFromRecord, ownerOccupancyToRecord,
  forecastScenarioFromRecord, forecastScenarioToRecord,
  forecastFlowComponentFromRecord, forecastFlowComponentToRecord,
  referenceDataFromRecord, referenceDataToRecord,
} from './mappers'
import type { Repositories } from '@/data/repositories'
import type { Property, Contact, SupplierContract, InvoiceTemplate, Attachment, ForecastFlow, ForecastFlowProperty, OwnerOccupancy, ForecastScenario, ForecastFlowComponent, ReferenceData } from '@/domain/types'
...
export const dataverseRepositories: Repositories = {
  properties: {
    ...
    async deletable(id) {
      const [invoices, contracts] = await Promise.all([
        Cr9b5_pt_invoicesService.getAll({ filter: `_cr9b5_property_value eq '${id}'`, select: ['cr9b5_pt_invoiceid'], maxPageSize: 5000 }),
        Svm_pt_suppliercontractsService.getAll({ filter: `_svm_pt_property_value eq '${id}'`, maxPageSize: 5000 }),
      ])
      const invoiceCount = (invoices.data ?? []).length
      const supplierContractCount = (contracts.data ?? []).length
      const deletable = invoiceCount === 0 && supplierContractCount === 0
      const reason = invoiceCount > 0
        ? `Cannot delete — this property has ${invoiceCount} invoice(s).`
        : supplierContractCount > 0
          ? `Cannot delete — this property has ${supplierContractCount} supplier contract(s).`
          : undefined
      return { deletable, reason, invoiceCount, supplierContractCount }
    },
  },
  ...
  referenceData: {
    async list() {
      const res = await Cr9b5_pt_referencesService.getAll({ orderBy: ['cr9b5_sortorder asc'], maxPageSize: 5000 })
      return (res.data ?? []).map(referenceDataFromRecord)
    },
    async save(r: ReferenceData | Omit<ReferenceData, 'id'>) {
      const rec = referenceDataToRecord(r)
      if ('id' in r && r.id) {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const res = await Cr9b5_pt_referencesService.update(r.id, rec as any)
        return referenceDataFromRecord(res.data!)
      }
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const res = await Cr9b5_pt_referencesService.create(rec as any)
      return referenceDataFromRecord(res.data!)
    },
    async remove(id) {
      await Cr9b5_pt_referencesService.delete(id)
    },
    async usageCount(id) {
      const [invoices, contracts, contacts, templates, flows] = await Promise.all([
        Cr9b5_pt_invoicesService.getAll({ filter: `_cr9b5_categoryid_value eq '${id}'`, select: ['cr9b5_pt_invoiceid'], maxPageSize: 5000 }),
        Svm_pt_suppliercontractsService.getAll({ filter: `_svm_defaultcategory_value eq '${id}'`, maxPageSize: 5000 }),
        Cr9b5_pt_contactsService.getAll({ filter: `_svm_defaultcategory_value eq '${id}'`, maxPageSize: 5000 }),
        Svm_pt_invoicetemplatesService.getAll({ filter: `_svm_category_value eq '${id}'`, maxPageSize: 5000 }),
        Cr9b5_pt_forecastflowsService.getAll({ filter: `_cr9b5_categoryid_value eq '${id}'`, maxPageSize: 5000 }),
      ])
      const byTable: Record<string, number> = {
        invoices: (invoices.data ?? []).length,
        supplier_contracts: (contracts.data ?? []).length,
        contacts: (contacts.data ?? []).length,
        invoice_templates: (templates.data ?? []).length,
        forecast_flows: (flows.data ?? []).length,
      }
  ...
  forecastFlows: {
    ...
    async save(f: ForecastFlow | Omit<ForecastFlow, 'id'>) {
      const rec: Record<string, unknown> = {
        ...
        cr9b5_notes: f.notes,
        svm_amountsource: f.amountSource,
        svm_percentage: f.percentage,
      }
  ...
  ownerOccupancies: {
    async list() {
      const res = await Svm_pt_owneroccupanciesService.getAll({ maxPageSize: 5000 })
      return (res.data ?? []).map(ownerOccupancyFromRecord)
    },
    async save(o: OwnerOccupancy | Omit<OwnerOccupancy, 'id'>) {
      const rec = ownerOccupancyToRecord(o)
      if ('id' in o && o.id) {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const res = await Svm_pt_owneroccupanciesService.update(o.id, rec as any)
        return ownerOccupancyFromRecord(res.data!)
      }
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const res = await Svm_pt_owneroccupanciesService.create(rec as any)
      return ownerOccupancyFromRecord(res.data!)
    },
    async remove(id) {
      await Svm_pt_owneroccupanciesService.delete(id)
    },
  },

  forecastScenarios: {
    async list() {
      const res = await Svm_forecastscenariosService.getAll({ maxPageSize: 5000 })
      return (res.data ?? []).map(forecastScenarioFromRecord)
    },
    async save(s: ForecastScenario | Omit<ForecastScenario, 'id'>) {
      const rec = forecastScenarioToRecord(s)
      if ('id' in s && s.id) {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const res = await Svm_forecastscenariosService.update(s.id, rec as any)
        return forecastScenarioFromRecord(res.data!)
      }
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const res = await Svm_forecastscenariosService.create(rec as any)
      return forecastScenarioFromRecord(res.data!)
    },
    async remove(id) {
      await Svm_forecastscenariosService.delete(id)
    },
  },

  forecastFlowComponents: {
    async list() {
      const res = await Svm_forecastflowcomponentsService.getAll({ maxPageSize: 5000 })
      return (res.data ?? []).map(forecastFlowComponentFromRecord)
    },
    async listBySourceFlow(flowId) {
      const res = await Svm_forecastflowcomponentsService.getAll({ filter: `_svm_sourceflow_value eq '${flowId}'` })
      return (res.data ?? []).map(forecastFlowComponentFromRecord)
    },
    async save(c: ForecastFlowComponent | Omit<ForecastFlowComponent, 'id'>) {
      const rec = forecastFlowComponentToRecord(c)
      if ('id' in c && c.id) {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const res = await Svm_forecastflowcomponentsService.update(c.id, rec as any)
        return forecastFlowComponentFromRecord(res.data!)
      }
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const res = await Svm_forecastflowcomponentsService.create(rec as any)
      return forecastFlowComponentFromRecord(res.data!)
    },
    async remove(id) {
      await Svm_forecastflowcomponentsService.delete(id)
    },
  },
