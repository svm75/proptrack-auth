...
// ---------- ReferenceData (unfiltered References rows) — added Step 8 ----------

export function referenceDataFromRecord(r: Cr9b5_pt_references): ReferenceData {
  return {
    id: r.cr9b5_pt_referenceid,
    value: r.cr9b5_value,
    referenceType: r.cr9b5_referencetype as unknown as number,
    sortOrder: r.cr9b5_sortorder ?? undefined,
  }
}

export function referenceDataToRecord(r: ReferenceData | Omit<ReferenceData, 'id'>): Record<string, unknown> {
  return {
    cr9b5_value: r.value,
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    cr9b5_referencetype: r.referenceType as any,
    cr9b5_sortorder: r.sortOrder,
  }
}
...
export function forecastFlowFromRecord(r: Cr9b5_pt_forecastflows): ForecastFlow {
  return {
    ...
    notes: r.cr9b5_notes ?? undefined,
    amountSource: ((r.svm_amountsource as unknown as number) ?? 925060000) as ForecastFlow['amountSource'],
    percentage: (r.svm_percentage as unknown as number) ?? undefined,
  }
}
...
export function ownerOccupancyFromRecord(r: Svm_pt_owneroccupancies): OwnerOccupancy {
  return {
    id: r.svm_pt_owneroccupancyid,
    name: r.svm_pt_name ?? undefined,
    propertyId: lookupValue(r, '_svm_pt_property_value') ?? '',
    fromDate: r.svm_pt_fromdate ?? '',
    toDate: r.svm_pt_todate ?? '',
    adults: r.svm_pt_adults ?? undefined,
    children: r.svm_pt_children ?? undefined,
    babies: r.svm_pt_babies ?? undefined,
  }
}

export function ownerOccupancyToRecord(o: Partial<OwnerOccupancy>): Record<string, unknown> {
  const rec: Record<string, unknown> = {}
  if (o.name !== undefined) rec.svm_pt_name = o.name
  if (o.fromDate !== undefined) rec.svm_pt_fromdate = o.fromDate
  if (o.toDate !== undefined) rec.svm_pt_todate = o.toDate
  if (o.adults !== undefined) rec.svm_pt_adults = o.adults
  if (o.children !== undefined) rec.svm_pt_children = o.children
  if (o.babies !== undefined) rec.svm_pt_babies = o.babies
  if (o.propertyId) rec['svm_pt_Property@odata.bind'] = `/cr9b5_pt_properties(${o.propertyId})`
  return rec
}

// ---------- ForecastScenario ----------

export function forecastScenarioFromRecord(r: Svm_forecastscenarios): ForecastScenario {
  return {
    id: r.svm_forecastscenarioid,
    name: r.svm_forecastscenario1 ?? '',
    propertyId: lookupValue(r, '_svm_property_value') ?? undefined,
    incomeAdjustmentPct: r.svm_incomeadjustmentpct ?? 0,
    expenseAdjustmentPct: r.svm_expenseadjustmentpct ?? 0,
    notes: r.svm_notes ?? undefined,
  }
}

export function forecastScenarioToRecord(s: Partial<ForecastScenario>): Record<string, unknown> {
  const rec: Record<string, unknown> = {}
  if (s.name !== undefined) rec.svm_forecastscenario1 = s.name
  if (s.incomeAdjustmentPct !== undefined) rec.svm_incomeadjustmentpct = s.incomeAdjustmentPct
  if (s.expenseAdjustmentPct !== undefined) rec.svm_expenseadjustmentpct = s.expenseAdjustmentPct
  if (s.notes !== undefined) rec.svm_notes = s.notes
  if (s.propertyId) rec['svm_Property@odata.bind'] = `/cr9b5_pt_properties(${s.propertyId})`
  return rec
}

// ---------- ForecastFlowComponent ----------

export function forecastFlowComponentFromRecord(r: Svm_forecastflowcomponents): ForecastFlowComponent {
  return {
    id: r.svm_forecastflowcomponentid,
    name: r.svm_name ?? undefined,
    sourceFlowId: lookupValue(r, '_svm_sourceflow_value') ?? '',
    targetFlowId: lookupValue(r, '_svm_targetflcow_value') ?? '',
    direction: (r.svm_direction as unknown as number) as ForecastFlowComponent['direction'],
  }
}

export function forecastFlowComponentToRecord(c: Partial<ForecastFlowComponent>): Record<string, unknown> {
  const rec: Record<string, unknown> = {}
  if (c.name !== undefined) rec.svm_name = c.name
  if (c.direction !== undefined) rec.svm_direction = c.direction
  if (c.sourceFlowId) rec['svm_SourceFlow@odata.bind'] = `/cr9b5_pt_forecastflows(${c.sourceFlowId})`
  if (c.targetFlowId) rec['svm_TargetFlcow@odata.bind'] = `/cr9b5_pt_forecastflows(${c.targetFlowId})`
  return rec
}
